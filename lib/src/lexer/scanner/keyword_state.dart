...
abstract class KeywordState {
  ...
  static KeywordState computeKeywordStateTable(
    ...
    List<KeywordState?> table =
        List<KeywordState?>.filled($z - $A + 1, /* fill = */ null);
    assert(length != 0);
    int chunk = 0;
    int chunkStart = -1;
    bool isLeaf = false;
    ...
    if (isLowercase) {
      ...
      return LowerCaseArrayKeywordState(table, syntax);
    } else {
      return UpperCaseArrayKeywordState(table, syntax);
    }
  }
}
...
abstract class ArrayKeywordState implements KeywordState {
  ...
  String toString() {
    ...
    for (int i = 0; i < foo.length; i++) {
      if (foo[i] != null) {
        sb.write("${String.fromCharCodes([i + $a])}: "
            "${foo[i]}; ");
      }
    }
    sb.write("]");
