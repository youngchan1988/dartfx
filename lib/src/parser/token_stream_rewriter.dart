...
import 'package:dartfx/src/lexer/tokens/comment_token.dart';
...
abstract class TokenStreamRewriter {
  ...
  Token insertParens(Token token, bool includeIdentifier) {
    ...
    BeginToken leftParen =
        next = SyntheticBeginToken(type: TokenType.OPEN_PAREN, offset: offset);
    ...
    next = _setNext(
        next, SyntheticToken(type: TokenType.CLOSE_PAREN, offset: offset));
  ...
  ReplacementToken replaceNextTokenWithSyntheticToken(
    ...
    ReplacementToken replacement = ReplacementToken(
        type: newTokenType, replacedToken: previousToken.next!);
  ...
  Token insertSyntheticKeyword(Token token, Keyword keyword) => insertToken(
      token,
      SyntheticKeywordToken(keyword: keyword, offset: token.next!.charOffset));
  ...
  Token insertSyntheticToken(Token token, TokenType newTokenType) {
    ...
    return insertToken(token,
        SyntheticToken(type: newTokenType, offset: token.next!.charOffset));
  }
