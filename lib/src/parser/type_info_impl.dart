...
class NoType implements TypeInfo {
  ...
  Token ensureTypeNotVoid(Token token, Parser parser) {
    parser.reportRecoverableErrorWithToken(token.next!, templateExpectedType);
...
class ComplexTypeParamOrArgInfo extends TypeParamOrArgInfo {
      ...
      if (allowsVariance &&
        ...
        while (identifier != null &&
          ...
          parser.reportRecoverableError(
              identifier, messageMultipleVarianceModifiers);
  ...
  Token parseMissingComma(Token token, Parser parser) {
    Token next = token.next!;
    parser.reportRecoverableError(
        next, templateExpectedButGot.withArguments(','));
    return parser.rewriter.insertToken(
        token, SyntheticToken(type: TokenType.COMMA, offset: next.charOffset));
  }
  ...
  Token parseUnexpectedEnd(Token token, bool isArguments, Parser parser) {
    ...
    if (optional('extends', next)) {
      if (!errorReported) {
        parser.reportRecoverableError(
            token, templateExpectedAfterButGot.withArguments('>'));
        errorReported = true;
      }
      token = next;
      next = token.next!;
    ...
    if (typeFollowsExtends || optional('Function', next)) {
      ...
      if (invalidType != noType) {
        ...
        parser.listener = ForwardingListener();
...
Token syntheticGt(Token next) {
  return SyntheticToken(type: TokenType.GT, offset: next.charOffset)
